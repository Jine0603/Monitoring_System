...
if ($startDate != '' && $endDate != '') {
    ...
    $conditions[] = "assigned_tbl.assigned_date = '$currentDate' AND assigned_tbl.status = '1'";
}

if ($cat != 'default') {
    $conditions[] = "assigned_tbl.cateid = '$cat' AND assigned_tbl.status = '1'";
}else{
    $conditions[] = "assigned_tbl.status = '1' AND assigned_tbl.assigned_date = '$currentDate'";
}
...
if (!empty($conditions)) {
    ...
    while ($rows = mysqli_fetch_assoc($query)) {
        ...
        $employee = $rows['employee_assigned'];
