...
function vpb_multiple_file_uploader(vpb_configuration_settings)
{
	...
	vpb_multiple_file_uploader.prototype.vpb_submit_added_files = function(){ 
		var uplod_balance = $("#upload_balance").val();
		var uplod_remarks = $("#upload_remarks").val();

		if(uplod_balance == '' || uplod_balance == ' ' || uplod_balance == null){
			Swal.fire(
				'Error',
				'Please Enter The ATM Balance',
				'error'
			  )
				}else{
					$('input[type="submit"]').hide();
	
					$('#close_mini_browser').show();


					$('#close_mini_browser').click(function() {
	...
	vpb_multiple_file_uploader.prototype.vasPLUS = function(file,file_counter)
	{
		if(typeof file[file_counter] != undefined && file[file_counter] != '')
		{
			...
			var uplod_reference = $("#upload_reference").val();
			var uplod_balance = $("#upload_balance").val();
			var uplod_remarks = $("#upload_remarks").val();
			...
			else
			{
				...
				dataString.append('upload_reference',uplod_reference);
				dataString.append('upload_balance',uplod_balance);
				dataString.append('upload_remarks',uplod_remarks);
